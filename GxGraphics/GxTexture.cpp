...
void Gx::Texture::reset(Gx::GraphicsDevice &device)
{
    ...
    HRESULT r = D3DXCreateTexture(device.device, d.size.width, d.size.height, d.mipLevels, gx_detail_d3d_usage(d.usage), gx_detail_d3d_format(d.format), gx_detail_d3d_pool(d.pool), &ptr);
    if(FAILED(r))
    {
